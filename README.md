...
`.nojekyll`). Most top-level `.html` files (index.html, dcr_intro.html,
...) have no front matter, so Jekyll passes them through unchanged at
their current URLs. `papers.html`, `talks.html`, `others.html`, and
`notejapanese.html` are the exception — they do have front matter and
are Liquid-rendered, listing the `_papers/`/`_talks/`/`_notes/`/
`_notes_ja/` collections.
...
date: YYYY-MM-DD
pdf: /Papers/my-paper.pdf      # optional, put the PDF in the existing Papers/ dir
...
It's published at `/papers/my-paper/`, grouped by `type` on `/papers.html`.
...
title on `/papers.html` links straight to `link`/`arxiv`/`pdf` instead
...
Add a Markdown file to `_notes/` (English, listed on `/others.html`) or
`_notes_ja/` (Japanese, listed on `/notejapanese.html`) — both share the
...
pdf: /Papers/my-note.pdf        # optional
tags: [example]
